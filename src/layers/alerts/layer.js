...
import { satellitesLayer } from '../satellites/layer.js'
...
// ── Notify helpers ────────────────────────────────────────────────────────────
function _notifyLogChange() {
  emitSessionEvent('geofence-alert', { latest: _alertLog.at(-1) || null })
  _onLogChange.forEach(fn => {
    try {
      fn([..._alertLog])
    } catch {
      /* ignore */
...
function _notifyGeofenceChange() {
  emitSessionEvent('geofences', { count: _geofences.length })
  _onGeofenceChange.forEach(fn => {
    try {
      fn([..._geofences])
    } catch {
      /* ignore */
...
function _notifyStatusChange(s) {
  _onStatusChange.forEach(fn => {
    try {
      fn(s)
    } catch {
      /* ignore */
...
function _buildCirclePositions(lon, lat, radiusKm, numPts = 64) {
  ...
  for (let i = 0; i <= numPts; i++) {
    ...
    positions.push(
      Cesium.Cartesian3.fromDegrees(lon + (dLon * 180) / Math.PI, lat + (dLat * 180) / Math.PI, 200)
    )
  }
  return positions
}
...
function _updateDrawPolyline() {
  ...
  _drawPolylineEntity = _viewer.entities.add({
    polyline: {
      positions: [..._drawVertices, _drawVertices[0]].map(([lo, la]) =>
        Cesium.Cartesian3.fromDegrees(lo, la, 200)
      ),
      width: 2,
      material: new Cesium.ColorMaterialProperty(
        Cesium.Color.fromCssColorString('#FFD700').withAlpha(0.5)
      ),
    },
  })
}
...
export const alertsLayer = {
  async activate({ viewer }) {
    ...
    this.addWatchSource({
      id: 'SATELLITES',
      getGeometrySnapshot: satellitesLayer.getGeometrySnapshot,
    })
    clearInterval(_timer)
    _timer = setInterval(() => _checkWatchSources(getTimeMs()), 1000)
  ...
  removeWatchSource(id) {
    _watchSources = _watchSources.filter(s => s.id !== id)
  },
  ...
  exportLog() {
    ...
    a.href = url
    a.download = 'dexearth_alerts.json'
    a.click()
  ...
  onLogChange(fn) {
    _onLogChange.push(fn)
    return () => {
      _onLogChange = _onLogChange.filter(f => f !== fn)
    }
  },
  onGeofenceChange(fn) {
    _onGeofenceChange.push(fn)
    return () => {
      _onGeofenceChange = _onGeofenceChange.filter(f => f !== fn)
    }
  },
  onStatus(fn) {
    _onStatusChange.push(fn)
    return () => {
      _onStatusChange = _onStatusChange.filter(f => f !== fn)
    }
  },
}
