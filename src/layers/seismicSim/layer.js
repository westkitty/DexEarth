...
function _nextId() {
  return `seismic_${++_idCounter}_${Date.now()}`
}
...
export const seismicSimLayer = {
  activate({ viewer }) {
    ...
    _timer = setInterval(() => _renderAll(getTimeMs()), 500)
  ...
  clearEvents() {
    _events = []
    emitSessionEvent('seismic', { events: [] }, 'LOCAL SIMULATION')
  },
  ...
  onTelemetry(fn) {
    _telemetryCallback = fn
    return () => {
      if (_telemetryCallback === fn) _telemetryCallback = null
    }
  },
  onStatus(fn) {
...
function _notifyStatus(s) {
  _statusCallbacks.forEach(fn => {
    try {
      fn(s)
    } catch {
      /* ignore */
...
export const PRESETS = {
  ...
  random: () => ({
    ...
    lon: [-145, -130, -106, -82, -73, -69, 143, 145, 150, -178, 171][
