...
function _notifyChange() {
  emitSessionEvent('cascade', { events: _activeEvents }, 'LOCAL SIMULATION')
  _onChange.forEach(fn => {
    try {
      fn(getState())
    } catch {
      /* ignore */
...
export const cascadeModel = {
  ...
  restoreEvents(ids) {
    this.reset()
    ids.forEach(id => this.triggerEvent(id))
  },
  getState() {
    return getState()
  },
  ...
  onChange(fn) {
    _onChange.push(fn)
    return () => {
      _onChange = _onChange.filter(f => f !== fn)
    }
  },
}
