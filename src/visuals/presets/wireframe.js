...
function buildLatLonGrid(viewer) {
  ...
  for (let lat = -90; lat <= 90; lat += 10) {
    ...
    const pts = []
    for (let lon = -180; lon <= 180; lon += 5) pts.push(Cesium.Cartesian3.fromDegrees(lon, lat, 80))
    col.add({
      positions: pts,
  ...
  for (let lon = -180; lon <= 180; lon += 10) {
    ...
    const pts = []
    for (let lat = -90; lat <= 90; lat += 5) pts.push(Cesium.Cartesian3.fromDegrees(lon, lat, 80))
    col.add({
      positions: pts,
      width: isMajor ? 1.2 : 0.8,
      material: Cesium.Material.fromType('Color', { color: isMajor ? majorColor : minorColor }),
