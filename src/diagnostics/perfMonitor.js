...
export function subscribePerf(fn) {
  ...
  fn({ ...perfState })
  return () => {
    _onUpdate = _onUpdate.filter(f => f !== fn)
  }
}
...
function _estimateEntities(telemetry) {
  ...
  for (const [layer, count] of Object.entries(telemetry)) {
    ...
    switch (layer) {
      case 'AIR_RADAR':
        ...
        break
      case 'ORBITAL_MATH':
        break // Count actual shared orbital collections below
      case 'THERMAL_FIRES':
        pts += count
        break
      case 'SEISMIC_SIM':
  ...
  const orbital = satellitesLayer.getRenderStats()
  pts += orbital.points
  lines += orbital.lines
  if (orbital.points > highestCount) offender = 'ORBITAL_MATH'
    ...
    if (_fpsSamples.length >= 60) {
      ...
      if (avg !== perfState.fps || Math.random() < 0.05) {
        ...
        if (!perfState.safeMode) {
          const overDraw =
            perfState.budgets.points.current > perfState.budgets.points.max ||
            ...
            try {
              ...
              if (overDraw && window._dexEarthTriggerSafeMode) {
                ...
                emitAudit(
                  'perf',
                  'SAFE_MODE_TRIGGERED',
                  `Geometry budget exceeded by ${perfState.topOffender}`
                )
              }
            } catch {
              /* ignore if not wired */
