...
export function buildLabelCollection({
  ...
  sortedFeatures.sort((a, b) => {
    ...
    const lenA =
      a.geometry?.coordinates?.[0]?.length || a.geometry?.coordinates?.[0]?.[0]?.length || 0
    const lenB =
      b.geometry?.coordinates?.[0]?.length || b.geometry?.coordinates?.[0]?.[0]?.length || 0
    return lenB - lenA
  })
  ...
  for (const feature of sortedFeatures) {
    ...
    col.add({
      ...
      fillColor: isHighlightedCountry
        ? Cesium.Color.YELLOW
        : isState
