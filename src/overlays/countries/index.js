...
function _removePrimitive(col) {
  if (col && _viewer) {
    try {
      _viewer.scene.primitives.remove(col)
    } catch {
      /* ignore */
    }
  }
...
export const countryLabelsLayer = {
  ...
  tick() {
    /* labels update via camera.moveEnd, not tick */
  },
  setStyle(style) {
    Object.assign(settings.labelStyle, style)
    _rebuildLabels()
  },
  setMaxLabels(n) {
    settings.maxLabels = n
    _rebuildLabels()
  },
}
...
export const countryFollowLabelsLayer = {
  ...
  setMaxLabels(n) {
    settings.maxFollowLabels = n
    _rebuildFollowLabels()
  },
  setOffset(px) {
    settings.followLabelOffset = px
    _rebuildFollowLabels()
  },
}
