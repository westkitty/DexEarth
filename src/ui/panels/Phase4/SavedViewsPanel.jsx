      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <CollapsibleSection title="📸 Saved Views" defaultOpen={false}>
          <div style={{ color: UI_TOKENS.textSecondary, fontSize: '10px', marginBottom: '8px' }}>
            Keyboard:{' '}
            <kbd style={{ background: '#333', padding: '1px 3px', borderRadius: '2px' }}>
              Alt+1..9
            </kbd>{' '}
            to load,{' '}
            <kbd style={{ background: '#333', padding: '1px 3px', borderRadius: '2px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: 1 }}>
                    <span
                      style={{
                        fontSize: '12px',
                        fontWeight: 'bold',
                        color: saved ? UI_TOKENS.textPrimary : UI_TOKENS.textMuted,
                      }}
                    >
                      {slot}
                    </span>
                    ...
                    {isEditing ? (
                      <input
                        ...
                        onKeyDown={e =>
                          e.key === 'Enter' && saveEdit(String(slot), saved.camera, saved.layers)
                        }
                      ...
                      <div
                        onClick={() =>
                          saved ? handleLoad(String(slot)) : handleSaveCurrent(String(slot))
                        }
                        style={{
                          flex: 1,
                          fontSize: '11px',
                  ...
                  <div style={{ display: 'flex', gap: '4px' }}>
                    {saved ? (
                      <>
                        <Button
                          variant="neutral"
                          onClick={() => startEdit(String(slot), saved.name)}
                        ...
                        <Button
                          variant="danger"
                          onClick={() => handleDelete(String(slot))}
                          style={{ padding: '2px 6px', fontSize: '9px' }}
                        >
                          ✕
                        </Button>
                      ...
                      <Button
                        variant="primary"
                        onClick={() => handleSaveCurrent(String(slot))}
                        style={{ padding: '2px 6px', fontSize: '9px' }}
                      >
                        SAVE
                      </Button>
