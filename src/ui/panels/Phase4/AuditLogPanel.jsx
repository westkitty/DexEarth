    ...
    <Panel style={{ position: 'relative' }}>
      <div style={{ position: 'relative', zIndex: 1 }}>
        <CollapsibleSection title="📄 Audit Log" defaultOpen={true}>
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ color: UI_TOKENS.textMuted }}>
                        {new Date(ev.timeMs).toLocaleTimeString()}
                      </span>
                      <StatusBadge status={statusColor} text={ev.category} />
                    </div>
                    <div style={{ color: UI_TOKENS.textPrimary, fontWeight: 'bold' }}>
                      {ev.event}
                    </div>
                    <div style={{ color: UI_TOKENS.textSecondary, overflowWrap: 'break-word' }}>
                      {ev.detail}
                    </div>
                  </div>
                )
          ...
          {/* Controls */}
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <Button onClick={clearAuditHistory} variant="danger">
              CLEAR LOGS
            </Button>
