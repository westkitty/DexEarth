...
export default function AlertsPanel({ viewer }) {
  const [isActive, setIsActive] = useState(alertsLayer.isActive)
  const [log, setLog] = useState(alertsLayer.getAlertLog)
  const [geofences, setGeofences] = useState(alertsLayer.getGeofences)
  useEffect(() => {
    const a = alertsLayer.onLogChange(setLog),
      b = alertsLayer.onGeofenceChange(setGeofences)
    return () => {
      a()
      b()
    }
    ...
    <div style={S.panel}>
      ...
      <div style={S.row}>
        <button
          style={{
            ...S.btn,
            ...(isActive ? { background: '#FFD70022', border: '1px solid #FFD700' } : {}),
          }}
          ...
          >
            <input
              ...
              onChange={e => {
                ...
                alertsLayer.setSoundEnabled(e.target.checked)
              }}
            />
            🔊 Sound
          </label>
      ...
      {isActive && (
        <>
          ...
          <div style={S.row}>
            <button style={S.btn} onClick={() => setAddMode('circle')}>
              + Circle
            </button>
            <button style={S.btn} onClick={startPolygon}>
              + Polygon
            </button>
          </div>
          ...
          {addMode === 'circle' && (
            ...
            >
              ...
              <div style={S.row}>
                <div>
                  ...
                  <input
                    style={S.input}
                    value={circleLon}
                    onChange={e => setCircleLon(e.target.value)}
                  />
                </div>
                <div>
              ...
              <div style={S.row}>
                <button style={S.btn} onClick={addCircle}>
                  ✓ Add
                </button>
                <button style={{ ...S.btn, color: '#FF4444' }} onClick={() => setAddMode(null)}>
                  Cancel
                </button>
              </div>
            </div>
          ...
          {addMode === 'polygon' && (
            ...
            >
              <div style={{ color: '#FFEEAA', fontSize: '10px', marginBottom: '4px' }}>
                Click globe to add vertices...
              </div>
              <input
                style={S.input}
                value={polyName}
              ...
              />
              <div style={S.row}>
                <button style={S.btn} onClick={finishPolygon}>
                  ✓ Finish
                </button>
                <button style={{ ...S.btn, color: '#FF4444' }} onClick={() => setAddMode(null)}>
                  Cancel
          ...
          {geofences.map(gf => (
            <div
              ...
              style={{
                ...
                borderBottom: '1px solid #FFD70011',
                paddingBottom: '2px',
              }}
            >
              <span style={{ color: '#FFEEAA', fontSize: '10px' }}>
                {gf.name} ({gf.type})
              </span>
              <button
                style={{ ...S.btn, padding: '1px 4px', color: '#FF4444', fontSize: '9px' }}
                onClick={() => alertsLayer.deleteGeofence(gf.id)}
              >
                ✕
              </button>
            </div>
          ...
          <div style={{ ...S.label, fontWeight: 'bold', marginTop: '6px' }}>
            Alert Log ({log.length})
          </div>
          <div style={S.row}>
            <button style={S.btn} onClick={() => alertsLayer.clearLog()}>
              Clear
            </button>
            <button style={S.btn} onClick={() => alertsLayer.exportLog()}>
              Export JSON
            </button>
          </div>
