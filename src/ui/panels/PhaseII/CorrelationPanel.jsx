...
export default function CorrelationPanel({ viewer }) {
  ...
  return (
    <div style={S.panel}>
      ...
      <div style={S.row}>
        <button style={S.btn} onClick={run} disabled={running}>
          {running ? '⏳ Running...' : '▶ Run'}
        </button>
        <button style={S.btn} onClick={() => correlationTool.clearHighlights()}>
          Clear Highlights
        </button>
        {result?.hitCount > 0 && (
          <button style={S.btn} onClick={() => correlationTool.export(result)}>
            Export JSON
          </button>
        )}
      </div>
      ...
      {result && (
        <div
          style={{
            fontSize: '10px',
            color: result.error ? '#FF4444' : '#88FFFF',
            marginTop: '4px',
          }}
        >
          {result.error || (
