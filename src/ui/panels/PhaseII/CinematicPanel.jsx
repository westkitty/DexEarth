...
export default function CinematicPanel({ viewer }) {
  ...
  useEffect(() => {
    ...
    return cinematicController.onStateChange(setState)
  }, [viewer])
  ...
  return (
    <div style={S.panel}>
      ...
      <div style={{ ...S.row, flexWrap: 'wrap' }}>
        {tours.map(tour => (
          <button
            ...
            style={{
              ...S.btn,
              ...(state.tourId === tour.id && state.playing
                ? { background: '#AAFFAA22', border: '1px solid #AAFFAA' }
                : {}),
            }}
      ...
      <div style={S.row}>
        {state.playing ? (
          <button style={S.btn} onClick={() => cinematicController.pause()}>
            ⏸ Pause
          </button>
        ) : (
          state.tourId && (
            <button style={S.btn} onClick={() => cinematicController.resume()}>
              ▶ Resume
            </button>
          )
        )}
        {state.tourId && (
          <>
            <button style={S.btn} onClick={() => cinematicController.skipForward()}>
              ⏭ Skip
            </button>
            <button
              style={{ ...S.btn, color: '#FF4444' }}
              onClick={() => cinematicController.stop()}
            >
              ■ Exit Tour
            </button>
          </>
      ...
      <div style={S.row}>
        ...
        <input
          ...
          onChange={e => {
            ...
            setSpeed(v)
            cinematicController.setSpeed(v)
          }}
          style={S.slider}
        />
      </div>
