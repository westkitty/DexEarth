...
export default function TimeControllerPanel({ viewer }) {
  ...
  return (
    <div style={{ ...STYLE.panel, position: 'relative' }}>
      ...
      >
        <div style={{ color: '#FFFFFF', fontSize: '11px', fontWeight: 'bold' }}>
          {tc.fmtUtc(displayTime)}
        </div>
      ...
      <div style={STYLE.row}>
        {['LIVE', 'MANUAL', 'REPLAY'].map(m => (
          <button key={m} style={btnStyle(mode === m)} onClick={() => handleMode(m)}>
            {m}
          </button>
        ))}
        <button
      ...
      {mode !== MODES.LIVE && (
        <div style={{ marginBottom: '4px' }}>
          ...
          />
          <div style={STYLE.row}>
            <button style={STYLE.btn} onClick={() => handleStep(-1)}>
              ◀ Step
            </button>
            <select
              style={STYLE.select}
      ...
      <div style={STYLE.row}>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer' }}>
          <input
            type="checkbox"
            checked={sunlight}
            onChange={e => handleSunlight(e.target.checked)}
          />
      ...
      {terminator && (
        <div style={{ paddingLeft: '8px' }}>
          ...
          <input
            ...
            min={1}
            max={10}
            value={Math.round(terminatorOpacity * 10)}
