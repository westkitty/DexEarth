...
import { deactivateSafeMode, activateSafeMode } from '../../../visuals/styleManager.js'
        ...
        <CollapsibleSection title="📊 Performance" defaultOpen={false}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <StatusBadge
                status={fpsColor(fps)}
                text={fps >= 45 ? 'SMOOTH' : fps >= 25 ? 'LAGGY' : 'CRITICAL'}
              />
            </div>
          </div>
          ...
          >
            ...
            >
              <span style={{ fontSize: '10px', color: UI_TOKENS.textSecondary }}>
                SAFE MODE POLICY
              </span>
              <StatusBadge
                status={safeMode ? 'warning' : 'neutral'}
                text={safeMode ? 'ACTIVE' : 'STANDBY'}
              />
            </div>
            {!safeMode && <Button onClick={activateSafeMode}>Force Safe Mode</Button>}
            {safeMode && (
