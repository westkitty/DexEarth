    ...
    <div style={{ padding: '4px 0', color: '#AAFFCC' }}>
      ...
      {follow && (
        <div style={{ paddingLeft: '16px', marginBottom: '6px' }}>
          ...
          <input
            ...
            step={5}
            value={followDensity}
            onChange={e => onFollowDensityChange(+e.target.value)}
            style={S.slider}
          />
        </div>
      )}
      ...
      {selected && (
        <div style={S.selected}>
          📍 {selected}
          <span
            style={{ marginLeft: '8px', cursor: 'pointer', color: '#FF4444' }}
            onClick={onClearSelection}
          >
            ✕
          </span>
          <button
            style={{ ...S.btn, marginLeft: '8px' }}
            onClick={() => flyToCountry(selected, viewer)}
          >
            Fly To
          </button>
        </div>
