import { toggleDrawer } from '../state/uiStore.js'
import ObservationsPanel from './panels/Phase4/ObservationsPanel.jsx'
import ReplayPanel from './panels/Phase4/ReplayPanel.jsx'
...
const SECTIONS = [
  {
    ...
    label: '⏺ Replay',
    color: '#ffd166',
    help: 'Local interaction recording. Not historical remote telemetry.',
  },
  {
    id: 'tools',
    label: '🧰 Tools',
    color: '#aaffaa',
    help: 'Markers, geofences, local cascade simulations, correlation and cinematic tours.',
  },
  {
    id: 'data',
    label: '🌐 Data Layers',
    color: '#00FF9F',
    help: 'Toggle primary tactical data layers including planes, shipping, and raw seismic events.',
  },
  {
    id: 'time',
    label: '🕰 Time & Orbit',
    color: '#00CFFF',
    help: 'Control playback speed, time sync, and evaluate orbital cascades based on Julian dates.',
  },
  {
    id: 'satellites',
    label: '🛰 Satellites',
    color: '#00FF9F',
    help: 'Inspect deterministic SGP4 positions derived from TLE elements. Bundled elements are not live telemetry.',
  },
  {
    id: 'seismic',
    label: '🌋 Seismic',
    color: '#FF8800',
    help: 'Track recent global seismic events and tectonic boundaries.',
  },
  {
    id: 'views',
    label: '📸 Views',
    color: '#AAFFAA',
    help: 'Save and manage scenario snapshots. Export offline views for later tactical reconstruction.',
  },
  {
    id: 'datasets',
    label: '📦 Datasets',
    color: '#00CFFF',
    help: 'Audit the staleness of data layers and manually override or cache them locally.',
  },
  {
    id: 'threat',
    label: '◉ Threat',
    color: '#FF2200',
    help: 'Algorithmic threat analysis overlay. Evaluates current geography against threat heuristics.',
  },
  {
    id: 'perf',
    label: '📊 Perf',
    color: '#FFFF00',
    help: 'Performance Governance. View system budgets and FPS. Automatically triggers Safe Mode if framerate drops.',
  },
  {
    id: 'audit',
    label: '📄 Log',
    color: '#AAAAAA',
    help: 'Rolling system audit log tracking layer toggles, errors, UI actions, and performance events.',
  },
  { id: 'help', label: 'ℹ️ Help', color: '#00FF9F', help: 'Access the Operator Manual.' },
  {
    ...
    label: '🎨 Visuals',
    color: '#FF00FF',
    help: 'Manage global render styles, procedural cloud systems, and country boundary overlays.',
  },
]
...
function TopNavTab({ id, label, color, help, open, onToggle, containerId, isMobile }) {
  return (
    <button
      ...
      onClick={() => onToggle(id)}
      style={{
        flex: isMobile ? '0 0 auto' : '1 0 85px',
        textAlign: 'center',
...
async function runDemoMode(viewer) {
  ...
  await markersLayer.addMarker({
    lon: 0.5,
    lat: 51.5,
    title: 'Demo: London Cable Landing',
    severity: 'warning',
    tags: ['demo'],
  })
  await markersLayer.addMarker({
  ...
  seismicSimLayer.addEvent({
    lon: 143.0,
    lat: 37.5,
    mag: 7.8,
    depthKm: 30,
    originMs: Date.now() - 300_000,
  })
...
export default function PhaseIIRoot({
  viewer,
  toggles,
  handleToggle,
  layerStatus,
  ...
  const bindDrag = useDrag(
    ...
    },
    {
      ...
      bounds: {
        left: 12,
        top: 12,
        right: Math.max(12, window.innerWidth - navWidth - 12),
  ...
  function toggleSection(id) {
    ...
    setOpenSections(prev => toggleDrawer(prev, id))
  }
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              background:
                'linear-gradient(90deg, transparent, rgba(0,255,159,0.05) 50%, transparent)',
          ...
          >
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              <span style={{ cursor: 'pointer' }} title="Drag to move Navigation Bar">
                ☷ DEXEARTH NAV
              </span>
              <button
                onClick={() => {
          ...
          {activeSection && (
            <div
              ...
              style={{
                ...
                height: 'min(45dvh, 460px)' /* Uniform pane size matching Log section */,
                overflowY: 'auto',
                position: 'relative',
                transition: 'all 150ms ease',
                    ...
                    <div key={sec.id}>
                      ...
                      >
                        ...
                        {(sec.id === 'data' || sec.id === 'time') && utc && (
                          <span
                            style={{ fontSize: '11px', color: '#888', fontFamily: 'monospace' }}
                          >
                            {utc}
                          </span>
                        )}
                      ...
                      {sec.id === 'satellites' && (
                        <SatellitesPanel viewer={viewer} toggleLayer={handleToggle} />
                      )}
                      ...
                      {sec.id === 'views' && (
                        <>
                          <ObservationsPanel />
                          <details>
                            <summary>Legacy saved view slots</summary>
                            <SavedViewsPanel viewer={viewer} toggles={toggles} />
                          </details>
                        </>
                      )}
                      {sec.id === 'sessions' && <ReplayPanel />}
                      {sec.id === 'tools' && (
                        <>
                          <MarkersPanel viewer={viewer} />
                          <AlertsPanel viewer={viewer} />
                          <CascadePanel />
                          <CorrelationPanel viewer={viewer} />
                          <CinematicPanel viewer={viewer} />
                        </>
                      )}
              ...
              <div style={{ display: 'grid', gap: '6px' }}>
                {SECTIONS.map(sec => (
                  <button
                    ...
                    style={{
                      textAlign: 'left',
                      background: openSections[sec.id]
                        ? 'rgba(0,255,159,0.1)'
